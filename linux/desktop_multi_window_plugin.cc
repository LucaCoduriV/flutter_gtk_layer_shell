...
static void desktop_multi_window_plugin_handle_method_call(
    ...
    if (g_strcmp0(method, "createWindow") == 0) {
        ...
        auto window_id = fl_value_get_int(fl_value_lookup_string(args, "windowId"));
        g_autoptr(FlValue)
        result = MultiWindowManager::Instance()->GetFrame(window_id);
        ...
        g_autoptr(FlValue)
        result = fl_value_new_bool(isMaximized);
        response = FL_METHOD_RESPONSE(fl_method_success_response_new(result));
        ...
        g_autoptr(FlValue)
        result = fl_value_new_bool(isPreventClose);
        response = FL_METHOD_RESPONSE(fl_method_success_response_new(result));
